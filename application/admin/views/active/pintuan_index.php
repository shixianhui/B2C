        ...
        foreach ($item_list as $item) {
            ...
            <tr id="id_<?php echo $item['id'] ?>"  onMouseOver="this.style.backgroundColor = '#ECF7FE'" onMouseOut="this.style.background = ''">
                ...
                <td class="align_c">
                    <?php
                     if($item['start_time'] < time() &&  $item['end_time'] > time()){ ?>
                     <a href="admincp.php/active/pintuan_save/<?php echo $item['id']; ?>">修改</a>
                    <?php }?>
                      <?php
                     if($item['end_time'] < time()){ ?>
                    <a href="admincp.php/active/pintuan_view/<?php echo $item['id']; ?>">查看</a>
                    <?php }?>
                </td>
            </tr>
        <?php } ?>
...
<script>
        ...
        if (con == true) {
            ...
            $.post(base_url + "admincp.php/" + controller + "/delete_pintuan",
