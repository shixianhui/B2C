...
<table class="table_form" cellpadding="0" cellspacing="1">
  <caption>基本信息</caption>
    ...
    <tr>
      ...
      <td>
      <input name="sell_price" id="sell_price" value="<?php if(! empty($item_info)){ echo $item_info['sell_price'];} ?>"  type="text" valid="required|isMoney" errmsg="出售价不能为空!|请填写正确的出售价格!"/> <font color="red">元</font> <font color="#999999">注：根据上面的价格属性列表，填写一个适中的价格</font>
      </td>
    </tr>
 	<tr>
    ...
    <tr>
        <th width="20%"><font color="red">*</font> <strong>成本价</strong> <br/>
        </th>
        <td>
            <input name="cost_price" id="cost_price" value="<?php if(! empty($item_info)){ echo $item_info['cost_price'];} ?>"  type="text" valid="required|isMoney" errmsg="成本价不能为空!|成本价必须为数字!"/> <font color="red">元</font>
        </td>
    </tr>
    <tr>
    ...
    <tr>
      <th width="20%"><font color="red">*</font> <strong>产品属性</strong> <br/>
	  </th>
      <td>
      <?php
      $index = 0;
      if ($product_type_arr) { ?>
      <?php foreach ($product_type_arr as $key=>$value) {
      	    $index++;
      	?>
     <label><input onclick="javascript:select_product_type();" <?php if ($index == count($product_type_arr)) {echo 'valid="requireChecked" errmsg="请选择产品属性"';} ?> type="radio" value="<?php echo $key; ?>" name="product_type" class="radio_style" <?php if($item_info){if($item_info['product_type']==$key){echo 'checked="checked"';}} ?> > <?php echo $value; ?></label>
      <?php }} ?>
      </td>
    </tr>
      <tr>
      ...
      <?php
          $give_score_str = '';
          if ($item_info) {
          	 if ($item_info['product_type'] == 'a') {
          	 	$give_score_str = '银象';
          	 } else if ($item_info['product_type'] == 'b') {
          	 	$give_score_str = '金象';
          	 } else if ($item_info['product_type'] == 'c') {
          	 	$give_score_str = '金象';
          	 }
          }
      ?>
      <strong class="give_score">送<?php echo $give_score_str; ?>积分数量</strong> <br/>
	  </th>
      <td>
      <input size="10" name="give_score" id="give_score" value="<?php if(! empty($item_info)){ echo $item_info['give_score'];}else{echo '0';} ?>"  type="text" valid="isNumber" errmsg="积分数量必须为数字!"/> <font color="red">个</font>
      </td>
    </tr>
     <tr>
      <th width="20%"><strong>积分换购</strong> <br/>
	  </th>
      <td>
      <input size="10" name="consume_score" id="consume_score" value="<?php if(! empty($item_info)){ echo $item_info['consume_score'];}else{echo '0';} ?>"  type="text" valid="isNumber" errmsg="积分数量必须为数字!"/>
       <font color="red">个</font>
      <font color="#999999">注：1、可以用多少个银/金象积分换购此商品；2、金象积分可换购 A、B、C类产品，银像积分可换购B、C类产品；3、银象积分优先使用；</font>
      </td>
    </tr>
    <tr>
      <th width="20%">
      <span style="color:red;">推广设置</span>
      </th>
      <td>
      <span style="color:red;">一级送积分</span>
      <span style="margin-left: 10px;color:red;">二级送积分</span>
      </td>
    </tr>
    <tr>
      <th width="20%"><font color="red">*</font> <strong>发展会员</strong> <br/>
	  </th>
      <td>
      <input size="6" name="divide_user_score" id="divide_user_score" value="<?php if(! empty($item_info)){ echo $item_info['divide_user_score'];} ?>"  type="text" valid="required|isInt" errmsg="发展会员一级可获得多少积分不能为空|请填写正确的积分数量"/>
      <input style="margin-left: 10px;" size="6" name="divide_user_score_sub" id="divide_user_score_sub" value="<?php if(! empty($item_info)){ echo $item_info['divide_user_score_sub'];} ?>"  type="text" valid="required|isInt" errmsg="发展会员二级可获得多少积分不能为空|请填写正确的积分数量"/>
      <font color="red">积分</font> <font color="#999999">注：发展的会员，消费后，一级、二级可得到多少积分</font>
      </td>
    </tr>
    <tr>
      <th width="20%"><font color="red">*</font> <strong>发展商家</strong> <br/>
	  </th>
      <td>
      <input size="6" name="divide_seller_score" id="divide_seller_score" value="<?php if(! empty($item_info)){ echo $item_info['divide_seller_score'];} ?>"  type="text" valid="required|isInt" errmsg="发展商家一级可获得多少积分不能为空|请填写正确的积分数量"/>
      <input style="margin-left: 10px;" size="6" name="divide_seller_score_sub" id="divide_seller_score_sub" value="<?php if(! empty($item_info)){ echo $item_info['divide_seller_score_sub'];} ?>"  type="text" valid="required|isInt" errmsg="发展商家二级可获得多少积分不能为空|请填写正确的积分数量"/>
      <font color="red">积分</font> <font color="#999999">注：发展的商家，消费后，一级、二级可得到多少积分</font>
      </td>
    </tr>
    <tr>
...
<table class="table_form" cellpadding="0" cellspacing="1">
  <caption>基本信息</caption>
 	<tbody>
 	<tr>
      <th width="20%"> <strong>产品简介</strong> <br/>
	  </th>
      <td>
      	<textarea maxlength="140" name="content_short" id="content_short" rows="4" cols="80"  class="textarea_style"><?php if(! empty($item_info)){ echo $item_info['content_short'];} ?></textarea>
      	<font color="red">注：最多140个字符</font>
      </td>
    </tr>
 	<tr>
