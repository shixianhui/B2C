...
<table cellpadding="2" cellspacing="1" class="table_form">
  ...
  <tr>
     ...
     <img id="max_previewpic" src="<?php if (isset($filePath)){echo preg_replace("/_thumb/", "_min", $filePath);} ?>" onerror="this.src='images/admin/nopic.gif'" />
	 <?php } ?>
	 </td>
   </tr>
</table>
...
$(function() {
	$('#image_upload').uploadify({
		...
		'onUploadSuccess' : function(file, data, response) {
			json = eval("(" + data + ")");
			if (json.success) {
				window.opener.document.getElementById("path").value = json.data.file_path;
				$("#previewpic").attr("src", json.data.file_path.replace('.', '_thumb.'));
				<?php if ($model == 'product') { ?>
				$("#max_previewpic").attr("src", json.data.file_path.replace('.', '_min.'));
				<?php } ?>
			} else {
                alert(json.message);
			}
