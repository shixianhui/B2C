<?php
class Elephant_log extends CI_Controller {
	...
	private $_tool = '';
	private $_table = '';
	private $_template = 'elephant_log';
	...
	public function __construct() {
		...
		//快捷方式
		$this->_tool = $this->load->view("element/elephant_log_tool", NULL, TRUE);
		//获取表对象
		...
		if ($_POST) {
			...
			if ($score_type == 'gold') {
				...
				if ($this->User_model->save($fields, array('id'=>$userInfo['id']))) {
					...
					printAjaxSuccess(base_url().'admincp.php/elephant_log/index/1/0/gold', '充值成功！');
				} else {
				...
				if ($this->User_model->save($fields, array('id'=>$userInfo['id']))) {
					...
					$this->tableObject->save($fields);
					printAjaxSuccess(base_url().'admincp.php/elephant_log/index/1/0/silver', '充值成功！');
				} else {
					printAjaxError('fail', "操作失败！");
				}
			}
		...
		if ($_POST) {
			...
			if ($score_type == 'gold') {
				...
				if ($this->User_model->save($fields, array('id'=>$userInfo['id']))) {
					...
					$this->tableObject->save($fields);
					printAjaxSuccess(base_url().'admincp.php/elephant_log/index/1/0/gold', '扣款成功！');
				} else {
					printAjaxError('fail', "操作失败！");
				}
			} else if ($score_type == 'silver') {
				...
				if ($this->User_model->save($fields, array('id'=>$userInfo['id']))) {
					...
					$this->tableObject->save($fields);
					printAjaxSuccess(base_url().'admincp.php/elephant_log/index/1/0/silver', '扣款成功！');
				} else {
					printAjaxError('fail', "操作失败！");
				}
			}
