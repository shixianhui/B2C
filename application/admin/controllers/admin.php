...
class Admin extends CI_Controller {
	...
	public function __construct() {
		parent::__construct();
		$this->_tool = $this->load->view("element/save_list_tool", array('table'=>$this->_table, 'parent_title'=>'员工管理', 'title'=>$this->_title), TRUE);
		//获取表对象
		$this->load->model(ucfirst($this->_template).'_model', 'tableObject', TRUE);
		$this->load->model('Admin_group_model', '', TRUE);
		$this->load->model('System_login_log_model', '', TRUE);
	...
	public function index($clear = 0, $page = 0) {
		...
		$admingroupList = $this->Admin_group_model->gets();
	...
	public function login() {
		if ($_POST) {
		    ...
		    if ($adminInfo) {
		        ...
		        if ($this->System_login_log_model->save($fields)) {
