...
class Product extends CI_Controller {
    ...
    public function index($clear = 0, $page = 0) {
        ...
        if ($_POST) {
            ...
            $divide_user_score = $this->input->post('divide_user_score', TRUE);
            $divide_seller_score = $this->input->post('divide_seller_score', TRUE);
            ...
            $stock_start = $this->input->post('stock_start', TRUE);
            $stock_end = $this->input->post('stock_end', TRUE);
            $select_category_id = $this->input->post('select_category_id');
            $postage_way_id = $this->input->post('postage_way_id', TRUE);
            $product_type = $this->input->post('product_type', TRUE);

            if ($product_type) {
            	$strWhere .= " and {$this->_table}.product_type = '{$product_type}' ";
            }
            if ($postage_way_id) {
            	$strWhere .= " and {$this->_table}.postage_way_id = {$postage_way_id} ";
            }
            if ($sales_start != '' && $sales_end != '') {
            	$strWhere .= " and ({$this->_table}.sales >= {$sales_start} and {$this->_table}.sales <= {$sales_end}) ";
            }
            if ($stock_start != '' && $stock_end != '') {
            	$strWhere .= " and ({$this->_table}.stock >= {$stock_start} and {$this->_table}.stock <= {$stock_end}) ";
            }
            if ($give_score != "") {
            	if ($give_score == 1) {
            		$strWhere .= " and {$this->_table}.give_score > 0 ";
            	} else {
            		$strWhere .= " and {$this->_table}.give_score = 0 ";
            	}
            }
            if ($consume_score != "") {
            	if ($consume_score == 1) {
            		$strWhere .= " and {$this->_table}.consume_score > 0 ";
            	} else {
            		$strWhere .= " and {$this->_table}.consume_score = 0 ";
            	}
            }
            if ($divide_user_score != "") {
            	if ($divide_user_score == 1) {
            		$strWhere .= " and {$this->_table}.divide_user_score > 0 ";
            	} else {
            		$strWhere .= " and {$this->_table}.divide_user_score = 0 ";
            	}
            }
            if ($divide_seller_score != "") {
            	if ($divide_seller_score == 1) {
            		$strWhere .= " and {$this->_table}.divide_seller_score > 0 ";
            	} else {
            		$strWhere .= " and {$this->_table}.divide_seller_score = 0 ";
            	}
            }
            ...
            if($select_category_id){
               $strWhere .= " and id in (select product_id from product_category_ids where product_category_id in ({$select_category_id})) ";
            }
    ...
    public function save($id = NULL) {
        ...
        if ($_POST) {
            ...
            /*************************推广开始***************************/
            $divide_user_score = $this->input->post('divide_user_score', TRUE);
            $divide_user_score_sub = $this->input->post('divide_user_score_sub', TRUE);
            $divide_seller_score = $this->input->post('divide_seller_score', TRUE);
            $divide_seller_score_sub = $this->input->post('divide_seller_score_sub', TRUE);
            /*************************推广结束***************************/
            ...
            /******************推广start********************/
            if (!$this->form_validation->required($divide_user_score)) {
            	printAjaxError('divide_user_score', '发展会员一级可获得多少积分不能为空');
            }
            if (!$this->form_validation->integer($divide_user_score)) {
            	printAjaxError('divide_user_score', '请填写正确的积分数量');
            }
            if (!$this->form_validation->required($divide_user_score_sub)) {
            	printAjaxError('divide_user_score_sub', '发展会员二级可获得多少积分不能为空');
            }
            if (!$this->form_validation->integer($divide_user_score_sub)) {
            	printAjaxError('divide_user_score_sub', '请填写正确的积分数量');
            }
            if (!$this->form_validation->required($divide_seller_score)) {
            	printAjaxError('divide_seller_score', '发展商家一级可获得多少积分不能为空');
            }
            if (!$this->form_validation->integer($divide_seller_score)) {
            	printAjaxError('divide_seller_score', '请填写正确的积分数量');
            }
            if (!$this->form_validation->required($divide_seller_score_sub)) {
            	printAjaxError('divide_seller_score_sub', '发展商家二级可获得多少积分不能为空');
            }
            if (!$this->form_validation->integer($divide_seller_score_sub)) {
            	printAjaxError('divide_seller_score_sub', '请填写正确的积分数量');
            }
            /******************推广end********************/
            ...
            $fields = array(
            	...
            	'divide_user_score'=>$divide_user_score,
            	'divide_user_score_sub'=>$divide_user_score_sub,
            	'divide_seller_score'=>$divide_seller_score,
            	'divide_seller_score_sub'=>$divide_seller_score_sub,
                'display_time' => time(),
            );
    ...
    public function delete() {
        ...
        if ($_POST) {
        	...
        	//收藏夹
        	if ($this->Product_favorite_model->rowCount("product_id in ({$ids})")) {
        		printAjaxError('fail','收藏夹存在关联数据，删除失败！');
        	}
        	if (!empty($ids)) {
