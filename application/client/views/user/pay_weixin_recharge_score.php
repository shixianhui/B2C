...
function weixin_heart() {
	$.post(base_url+"index.php/"+controller+"/get_weixin_heart_recharge_score",
			{	"out_trade_no":'<?php echo $out_trade_no; ?>'
			},
			function(res){
				if(res.success) {
					if (res.data.trade_status != 'WAIT_BUYER_PAY') {
	                    window.location.href = base_url+'index.php/'+controller+'/pay_result_recharge_score/'+res.data.id+'.html';
					}
				}
			},
			"json"
