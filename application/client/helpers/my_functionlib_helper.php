 ...
 * @return	string
 */
if ( ! function_exists('get_qrcode_logo')) {
	function get_qrcode_logo($url = NULL, $logo = NULL) {
		include 'phpqrcode/phpqrcode.php';
		$errorCorrectionLevel = 'H';//容错级别
...
if ( ! function_exists('download_qrcode')) {
	function download_qrcode($url = NULL, $logo = NULL, $matrixPointSize = 6) {
		include 'phpqrcode/phpqrcode.php';
		$errorCorrectionLevel = 'H';//容错级别
		//生成二维码图片
		$QR = QRcode::png($url, 2, $errorCorrectionLevel, $matrixPointSize, 2);
		$image_arr = fliter_image_path($logo);
		$logo = $image_arr['path_thumb'];
		if ($logo) {
			$logo = imagecreatefromstring(file_get_contents($logo));
			$QR_width = imagesx($QR);//二维码图片宽度
			$QR_height = imagesy($QR);//二维码图片高度
 ...
 *
 */
if ( ! function_exists('fliter_image_path')) {
	function fliter_image_path($image_path = NULL) {
		$path = '';
        $path_thumb = '';
        if ($image_path) {
...
if ( ! function_exists('get_distributor_info')) {
	function get_distributor_info($user_id = NULL) {
		$CI = & get_instance();
		return $CI->advdbclass->get_distributor_info($user_id);
	}
}
