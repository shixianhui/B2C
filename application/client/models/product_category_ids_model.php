...
class Product_category_ids_model extends CI_Model {
	...
	public function get_product_count($strWhere = NULL) {
		$ret = array();
		$this->db->select("{$this->_tableName}.product_id");
		$this->db->distinct();
	...
	public function gets2($strWhere = NULL) {
		$ret = array();
		$this->db->select("{$this->_tableName}.parent_id, {$this->_productCategoryTName}.product_category_name,{$this->_productCategoryTName}.id");
